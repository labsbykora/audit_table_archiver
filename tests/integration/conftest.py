...
@pytest.fixture
async def test_data(
    db_connection: asyncpg.Connection, test_table: str
) -> AsyncGenerator[list[dict], None]:
    """Insert test data and return records."""
    from datetime import datetime, timedelta, timezone

    # Insert test records (some old, some new)
    now = datetime.now(timezone.utc)
    old_date = now - timedelta(days=100)  # 100 days ago (eligible for archival)
...
@pytest.fixture
def s3_client(s3_config: S3Config, minio_ready: None) -> S3Client:
    """Create S3 client for testing."""
    # Set up MinIO credentials via environment
    ...
    try:
        client.client.create_bucket(Bucket=s3_config.bucket)
    except Exception:
